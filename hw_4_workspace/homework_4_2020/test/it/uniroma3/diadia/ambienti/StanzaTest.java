...
public class StanzaTest {
	...
	private Stanza impostaStanzeAdiacenti(String[] direzioni, Stanza[] adiacenti) {
		...
		for(int i = 0; i < direzioni.length; i++) {
			Direzione direzione = Direzione.valueOf(direzioni[i].toUpperCase());
	...
	public void testSovrascriviStanzaAdiacente() {
		...
		assertTrue(
				"Stanza adiacente a nord sovrascritta",
				stanza.impostaStanzaAdiacente(NORD, new Stanza("Nuova Stanza adiacente a nord"))
			);
		assertEquals("Nuova Stanza adiacente a nord\nUscite: \n", stanza.getStanzaAdiacente(NORD).toString());
	}

	@Test (expected=IllegalArgumentException.class)
	public void testImpostaStanzaAdiacenteFallisce() {
		// Stanza stanza = 
		this.impostaStanzeAdiacenti(
				new String[] {"nord", "sud", "ovest", "alto"},
				new Stanza[] {
	...
	public void testSetPersonaggio() {
		...
		Stanza stanza = new Stanza("Stanza");
		assertNull(stanza.getPersonaggio());
		stanza.setPersonaggio(personaggio);
		assertNotNull(stanza.getPersonaggio());
		assertEquals(personaggio.getClass(), Personaggio.class);
		assertEquals(personaggio, stanza.getPersonaggio());
	}
}
